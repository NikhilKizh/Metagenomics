...
#PBS -N s12a_q2_ancom_filter_table.sh
...
qiime feature-table filter-features \
  ...
  --p-min-frequency 10 \
  --p-min-samples 2 \
  --o-filtered-table "${results_folder}/s12a_table_2k_abund.qza"

# adding pseudo_counts

qiime composition add-pseudocount \
  --i-table "${results_folder}/s12a_table_2k_abund.qza" \
  --o-composition-table "${results_folder}/s12a_filter_pseudo_table_abund_comp.qza"
